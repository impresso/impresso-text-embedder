...
The model's max sequence is 8192 tokens. Two orthogonal registries handle everything past that boundary: `impresso_text_embedder.chunking` (text → K chunk strings) and `impresso_text_embedder.aggregation` (K chunk vectors → 1 doc vector). Both accept kwargs so new strategies plug in without touching callers. Full design space in `.progress/long-doc-chunking/notes.md`; registry contract in `.progress/chunking/notes.md`.

Chunking strategies registered today (`src/impresso_text_embedder/chunking/__init__.py`):
- **`fixed-window`** (CLI default for `--embedding-level=text` long-doc path). Tokenize with the model's own tokenizer → slice the flat id list into contiguous non-overlapping windows of `max_tokens` (auto-derived as `tokenizer.model_max_length - num_special_tokens_to_add(pair=False)` → 8190 for gte-multilingual-base) → decode each slice back to a string. No overlap, no sentence awareness. ~20 LOC in `chunking/fixed_window.py`. Known redundancy: encoder re-tokenises each decoded string (L1 in the notes); kept deliberately for simplicity.
- **`token-budget`** (registered, opt-in — not wired to the CLI today). Sentence-aware greedy packer: split on `.!?\s` → accumulate sentences until adding the next would exceed `max_tokens` → flush. Over-budget sentences emit as solo chunks (encoder truncates). `chunking/token_budget.py`. Reachable via `get_strategy("token-budget", token_counter=…, max_tokens=…)` for A/B testing once `fixed-window` has a baseline.
- **`semantic`** (registered, opt-in — wired only via `--embedding-level=chunk --chunking-strategy=semantic`). Ports old behaviour: `chonkie.SemanticChunker(embedding_model="minishlab/potion-base-8M", threshold=0.5, chunk_size=1024, min_sentences=5)`. Lazy-imported so chonkie isn't pulled in until the strategy is actually used. `chunking/semantic.py`.

Aggregation strategies registered today (`src/impresso_text_embedder/aggregation/__init__.py`):
- **`mean`** (the only one wired; `--long-doc-aggregation=mean`). Unweighted mean over K chunk vectors, followed by L2 renormalisation — because the mean of K unit vectors is not itself unit-norm. Logs a WARNING when the pre-renorm norm collapses below `1e-6` (chunks nearly orthogonal → noise amplification). `aggregation/mean.py`. Matches the PCW baseline in LongEmbed (arxiv 2404.12096).
- **Deferred:** length-weighted, max-pool, first-chunk, etc. — each lands as a module + one `register_strategy` line + one `choices=` tuple entry in `cli/create.py`.

CLI wiring (long-doc path, `--embedding-level=text`): `--long-doc-strategy {truncate,chunk}` (default `chunk`; `truncate` reproduces pre-step-16 outputs), `--long-doc-chunk-tokens N` (auto-derived when omitted), `--long-doc-aggregation {mean}`. Chunking only fires when `tokens(doc) > model_max_tokens` — for docs ≤8192 the one-shot path wins (see the boundary-decision paragraph in the final "Decisions recorded" entry for why).
...
- Other flags, carried over or adapted from the old CLI: `--model-name`, `--model-revision`, `--embedding-level {text,sentence,chunk}`, `--batch-size`, `--min-char-length`, `--content-type {ar,page}`, `--no-overwrite`, `--skip-if-s3-exists` (default on), `--dry-run`. Per-chunk L2 normalisation is no longer a CLI knob — see the `--normalize-embeddings` removal entry under "Decisions recorded".
...
- `--source <path>` — original input `.jsonl.bz2` (local or `s3://`). When combined with `--target`, computes per-direction statistics on records that show up as `above tolerance`, `missing in target`, or `missing in produced`: char-length distribution + log-scale histogram, `lg` / `tp` breakdowns, below-`--source-min-char-length` count (default `400`, matching `impresso-embed-create`), `empty` (no `ft` and no reconstructable `sents`) count, `not in source` drift tally, and sample excerpts. For the `above tolerance` panel the 3 samples are the **worst drifts** by cosine distance (descending), each annotated `d=<distance>`; for the missing panels samples are the first 3 encountered. Without `--target`, emits a WARNING on stderr and is otherwise ignored.
- Output uses Rich (tables + panels) with ANSI auto-stripped when piped or captured, so CI logs and `capsys`-based tests see plain text. Legacy substring markers (`records_checked=`, `MISMATCH:`, `p50=`, `log10(distance)`, `worst drifts`, `mismatches`, `missing in target (N)`, `OK`, `FAIL`) are preserved alongside the Rich rendering so existing grep tooling keeps working.
...
│       ├── chunking/           # pluggable text → K-chunks strategies
│       │   ├── base.py         # Chunk / ChunkingStrategy / registry
│       │   ├── fixed_window.py # tokenizer-slice windows — CLI default (long-doc path)
│       │   ├── token_budget.py # sentence-aware greedy packer (opt-in)
│       │   └── semantic.py     # chonkie SemanticChunker (chunk-level output)
│       ├── aggregation/        # pluggable K-vectors → 1-vector strategies
│       │   ├── base.py         # AggregationStrategy / registry
│       │   └── mean.py         # mean-pool + L2 renorm — only one wired today
