...
# re-computations by different machines working on the dataset are less likely to happen.
...
#s3 = m.get_s3_resource(); bucket = s3.Bucket('$(IN_S3_BUCKET_REBUILT)'); \
#result = bucket.meta.client.list_objects_v2(Bucket=bucket.name, Prefix='$(IN_S3_PREFIXES)/', Delimiter='/'); \
#print('DEBUG: Prefix used = $(IN_S3_PREFIXES)/'); \
#print('DEBUG: CommonPrefixes =', result.get('CommonPrefixes')); " \
#	> $@
$(NEWSPAPERS_TO_PROCESS_FILE):
	python -c \
	"import lib.s3_to_local_stamps as m, random; \
s3 = m.get_s3_resource(); bucket = s3.Bucket('$(IN_S3_BUCKET_REBUILT)'); \
level1 = bucket.meta.client.list_objects_v2(Bucket=bucket.name, Prefix='$(IN_S3_PREFIXES)/', Delimiter='/').get('CommonPrefixes') or []; \
pairs = []; \
[ pairs.extend([ p['Prefix'][len('$(IN_S3_PREFIXES)/'):-1] + '/' + x['Prefix'][len(p['Prefix']):-1] \
    for x in (bucket.meta.client.list_objects_v2(Bucket=bucket.name, Prefix=p['Prefix'], Delimiter='/').get('CommonPrefixes') or []) ]) \
    for p in level1 ]; \
...
IN_S3_PATH_REBUILT := s3://$(IN_S3_BUCKET_REBUILT)/$(IN_S3_PREFIXES)/$(PROVIDER)/$(NEWSPAPER)
  $(call log.debug, IN_S3_PATH_REBUILT)

# The local path
#IN_LOCAL_PATH_REBUILT := $(BUILD_DIR)/$(IN_S3_BUCKET_REBUILT)/$(NEWSPAPER)
IN_LOCAL_PATH_REBUILT := $(BUILD_DIR)/$(IN_S3_BUCKET_REBUILT)/$(IN_S3_PREFIXES)/$(PROVIDER)/$(NEWSPAPER)
...
COMBINED_FOLDER := $(OUT_S3_PROCESSED_INFIX)-$(EMBEDDING_LEVEL_OPTION)-$(OUT_S3_PROCESSED_VERSION)

# The s3 output path
OUT_S3_PATH_PROCESSED_DATA := s3://$(OUT_S3_BUCKET_PROCESSED_DATA)/$(COMBINED_FOLDER)/$(PROVIDER)/$(NEWSPAPER)
  $(call log.debug, OUT_S3_PATH_PROCESSED_DATA)

# The local path in BUILD_DIR
OUT_LOCAL_PATH_PROCESSED_DATA := $(BUILD_DIR)/$(OUT_S3_BUCKET_PROCESSED_DATA)/$(COMBINED_FOLDER)/$(PROVIDER)/$(NEWSPAPER)
...
# function to turn a local file path into a s3 file path, optional cutting off the
